...
def generate_trades_for_player(player, player_index, num_weeks=16):
    ...
    # Ensure first 20 trades are buys
    for i in range(20):
        gamer_group = random.choice(gamer_stake_ranges)
        gamer = random.choice(gamer_group["gamers"])
        trade_datetime = start_date + timedelta(days=i, hours=random.randint(0, 23), minutes=random.randint(0, 59), seconds=random.randint(0, 59))
    ...
    for week in range(num_weeks):
        num_trades = random.randint(30, 50)  # 30-50 trades per week
...
print("Task completed. Data exported to 'TSW2_FSM_Trade.csv'")